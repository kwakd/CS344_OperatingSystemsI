...
{
    int   inputExecute, outputExecute, result;
    ...
    else if (pid == 0) //if we get 0 from fork() we were able to return t
    {
      ...
      if(strcmp(input, "")) //makes sure file name isnt blank
      {
        ...
        if(input == -1) //file fails to open it sends back -1
        {
            perror("Can't open input file\n");
            exit(1); //exits
        }

        result = dup2(input, 0); //Resource 6: Input & Output Redirection, makes sure input file can be read
      ...
      if(strcmp(output, ""))
      {
          output = open(output, O_WRONLY | O_CREAT | O_TRUNC, 0666); //Opens the file with O_WRONLY
          if(output == -1) //file cant be opened
          {
            perror("Can't open output file\n");
            exit(1);
          }
          result = dup2(output, 1); //setting up for output //https://man7.org/linux/man-pages/man2/dup.2.html
